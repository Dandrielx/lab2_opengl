...
from matrices import create_translation_matrix, create_rotation_matrix_y
...
def draw_scene(camera, projection_mode):
    ...
    for row in range(-4, 12, 4):
        for col in range(-4, 16, 4):
            glPushMatrix()
            glMultMatrixf(create_translation_matrix(row, 0, col-2.5).T)
            glMultMatrixf(create_rotation_matrix_y(180).T)
            draw_chair()
            glPopMatrix()
